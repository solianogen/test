    ...
    <ul class="navbar-nav mr-auto">
      ...
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu" aria-labelledby="navbarDropdown">
          ...
          <div class="dropdown-divider"></div>
          <a class="dropdown-item hover" href="#">Pending</a> 
        </div>
      </li>
      <li class="nav-item dropdown">
...
<br>
    <center>
      <h1 style="font-family: Century Gothic,Arial; color:maroon">My Pending Reservations</h1>
      <?php
        $result = mysqli_query($db, "SELECT SUM(grade1.price * reservation.purchaseqty) AS total FROM grade1 INNER JOIN reservation ON grade1.id=reservation.bookid WHERE reservation.status='Pending' AND reservation.idnumber='{$_SESSION['idnumber']}'");
        $data = mysqli_fetch_array($result);
          if(mysqli_num_rows($result) > 0)
          {
            echo "<h1 class='text-danger'>₱ {$data['total']}</h1>";
            echo "<h3 class='text-muted'>Total</h3>";
          }
          else
          {
            echo "<h1 class='text-danger'>₱ 0.00</h1>";
            echo "<h3 class='text-muted'>Total</h3>"; 
          }
   ...
   <?php
    ...
    $result = mysqli_query($db, "SELECT * FROM  grade1 INNER JOIN reservation ON grade1.id=reservation.bookid WHERE reservation.idnumber = $idnumber AND reservation.status = 'pending'");
?>
    <div class="table">
      ...
      while($res = mysqli_fetch_array($result))
      {
        ...
        echo "<td align='center'><a href=\"delete.php?purchaseid={$res['purchaseid']}\" onClick=\"return confirm('Are you sure you want to delete?')\">Cancel</a></td>";
      }
      ?>
    </table>
</body>
